...
def get_weight_mapping(config: NanotronLlamaConfig, nt_to_hf: bool = True) -> dict[str, str]:
    ...
    for i in range(config.num_hidden_layers):
        ...
        hf_to_nt_map[f"{hf_prefix}.self_attn.lambda_q1"] = f"{nt_prefix}.attn.lambda_q1"
        hf_to_nt_map[f"{hf_prefix}.self_attn.lambda_q2"] = f"{nt_prefix}.attn.lambda_q2"
        hf_to_nt_map[f"{hf_prefix}.self_attn.lambda_k1"] = f"{nt_prefix}.attn.lambda_k1"
        hf_to_nt_map[f"{hf_prefix}.self_attn.lambda_k2"] = f"{nt_prefix}.attn.lambda_k2"
        hf_to_nt_map[f"{hf_prefix}.self_attn.subln.weight"] = f"{nt_prefix}.attn.subln.weight"
